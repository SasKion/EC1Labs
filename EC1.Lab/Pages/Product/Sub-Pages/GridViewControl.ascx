<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="GridViewControl.ascx.cs" Inherits="GalaxyShoes.Pages.Product.Sub_Pages.GridViewControl" %>
...
<asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:GalaxyShoes %>" SelectCommand="SELECT * FROM [Product]" DeleteCommand="DELETE FROM [Product] WHERE [ProductId] = @ProductId" InsertCommand="INSERT INTO [Product] ([ProductId], [Name], [ShortDescription], [UnitPrice], [ImageFile], [InStock]) VALUES (@ProductId, @Name, @ShortDescription, @UnitPrice, @ImageFile, @InStock)" UpdateCommand="UPDATE [Product] SET [Name] = @Name, [ShortDescription] = @ShortDescription, [UnitPrice] = @UnitPrice, [ImageFile] = @ImageFile, [InStock] = @InStock WHERE [ProductId] = @ProductId">
    <DeleteParameters>
